...
export const HouseholdModal: React.FC<HouseholdModalProps> = ({
  ...
  // 世帯主の氏名・ふりがな自動補完フック
  const {
    isCustomized: isHeadFuriganaCustomized,
    resetFurigana: resetHeadFurigana,
    nameInputProps: headNameInputProps,
    furiganaInputProps: headFuriganaInputProps,
  } = useAutoKana({
    nameValue: formData.familyHead || '',
    furiganaValue: formData.furigana || '',
    onNameChange: (val) => setFormData((prev) => ({ ...prev, familyHead: val })),
    onFuriganaChange: (val) => setFormData((prev) => ({ ...prev, furigana: val })),
  });
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="font-bold text-[#1A1A1A]">世帯主 *</label>
                  <span className="text-[10px] text-[#D4AF37] font-sans flex items-center gap-0.5">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="font-bold text-[#444444]">ふりがな</label>
                  {isHeadFuriganaCustomized && formData.furigana && (
                    <button
                      type="button"
                      onClick={resetHeadFurigana}
                      className="text-[10px] text-[#888888] hover:text-[#1A1A1A] underline cursor-pointer"
                      title="ふりがなを自動再同期"
                    >
                      自動再同期
                    </button>
              ...
              <div className="space-y-3">
                {familyMembers.map((member, idx) => (
                  <FamilyMemberInputRow
                    ...
                    configuredTobaSlots={configuredTobaSlots}
                    onUpdateField={(field, value) => handleUpdateFamilyMember(idx, field, value)}
                    onUpdateMember={(updatedMem) => {
                      const updatedList = [...familyMembers];
                      updatedList[idx] = updatedMem;
                      setFamilyMembers(updatedList);
                    }}
                    onRemove={() => handleRemoveFamilyMember(idx)}
                  />
                ))}
              </div>
            )}
