...
export async function importFromExcel(
  ...
  if (infoRows.length > 0) {
    ...
    templeInfo = {
      ...
      sect: infoMap.get('宗派') || '',
      mountainName: infoMap.get('山号') || '',
      chiefPriest: infoMap.get('住職名') || '',
      postalCode: infoMap.get('郵便番号') || '',
      address: infoMap.get('住所') || '',
      phone: infoMap.get('電話番号') || '',
      fax: infoMap.get('FAX番号') || '',
      website: infoMap.get('ホームページ') || '',
      bankInfo: infoMap.get('銀行振込口座') || '',
